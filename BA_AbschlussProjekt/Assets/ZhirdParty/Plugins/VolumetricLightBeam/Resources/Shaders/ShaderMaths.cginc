...
#define _VLB_SHADER_MATHS_INCLUDED_
    ...
    inline float4 VLBObjectToClipPos(in float3 pos) { return mul(UNITY_MATRIX_MVP, float4(pos, 1.0)); }
        ...
        #define matWorldToObject    UNITY_ACCESS_INSTANCED_PROP(Props, _WorldToLocalMatrix)
        #define matObjectToWorld    UNITY_ACCESS_INSTANCED_PROP(Props, _LocalToWorldMatrix)
        inline float4 VLBObjectToClipPos(in float3 pos) { return mul(mul(unity_MatrixVP, matObjectToWorld), float4(pos, 1.0)); }
    #else
...
inline float3 VLBWorldToObjectPos(in float3 pos)    { return mul(matWorldToObject, float4(pos, 1.0)).xyz; }
//inline float3 VLBObjectToWorldPos(in float3 pos)    { return mul(matObjectToWorld, float4(pos, 1.0)).xyz; } // unused
